...
def resnet18(pretrained=True, **kwargs):
    ...
    model = ResNet(BasicBlock, NLblock2d, [2, 2, 2, 2], nl=True)
    if pretrained:
...
def resnet34(pretrained=True, **kwargs):
    ...
    model = ResNet(BasicBlock, NLblock2d, [3, 4, 6, 3], nl=True)
    if pretrained:
        state_dict = model_zoo.load_url(model_urls["resnet152"])
        state_dict.pop("fc.weight")
        state_dict.pop("fc.bias")
...
def resnet50(pretrained=True, **kwargs):
    ...
    model = ResNet(BottleNeck, NLblock2d, [3, 4, 6, 3], nl=True)
    if pretrained:
        state_dict = model_zoo.load_url(model_urls["resnet152"])
        state_dict.pop("fc.weight")
        state_dict.pop("fc.bias")
...
if __name__ == "__main__":
    ...
    backbone = resnet101(num_classes=20)
    backbone.cuda()
    x = torch.rand([2, 3, 256, 256], device="cuda")
    features = backbone.forward(x)
    assert features.shape == (2, 20)

    backbone = resnet152(num_classes=20)
    backbone.cuda()
    x = torch.rand([2, 3, 256, 256], device="cuda")
    features = backbone.forward(x)
    assert features.shape == (2, 20)
