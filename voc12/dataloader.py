...
def load_image_label_from_xml(img_name, voc12_root):
    ...
    elem_list = minidom.parse(
        os.path.join(
            voc12_root, ANNOT_FOLDER_NAME, decode_int_filename(img_name) + ".xml"
        )
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.resize_long:
            img = imutils.random_resize_long(
                img, self.resize_long[0], self.resize_long[1]
            )
...
class VOC12PseudoSegmentationDataset(VOC12ClassificationDataset):
    ...
    def update_cam(self, idx, cam):
        ...
        conf = keys[pred]
        # unique_elements, counts_elements = np.unique(conf, return_counts=True)
        # percentage = counts_elements / len(pred.flatten())
        # for i, j in zip(unique_elements, percentage):
        #     if j < 0.05:
        #         conf = np.where(conf == i, 0, conf)
    ...
    def generate_label(self, idx, img):
        ...
        if os.path.exists(path):
            ...
            return np.zeros((img.shape[0], img.shape[1])).astype(np.uint8)

    def _get_label(self, name_str):
        label_path = os.path.join(
            self.voc12_root, "SegmentationClass", name_str + ".png"
        )
    ...
    def __getitem__(self, idx):
        ...
        try:
            original_label = self._get_label(name_str)
        except:
            original_label = seg_label
        if self.resize_long:
            img, seg_label = imutils.random_resize_long(
                (img, seg_label), self.resize_long[0], self.resize_long[1]
            )

        if self.rescale:
            img, seg_label = imutils.random_scale(
                (img, seg_label), scale_range=self.rescale, order=(3, 0)
            )
        ...
        if self.crop_size:
            if self.crop_method == "random":
                img, seg_label = imutils.random_crop(
                    (img, seg_label), self.crop_size, (0, 0)
                )
            else:
                img = imutils.top_left_crop(img, self.crop_size, 0)
        ...
        return {
            ...
            "img": img,
            "idx": idx,
            "seg_label": seg_label.astype(np.long),
            "label": label,
...
#
# print(mean(shape0), min(shape0), max(shape0))
# print(mean(shape1), min(shape1), max(shape1))
